...
save_folder_name = "./result/train/" + execution_time
...
def main():
    ...
    logprint("=== Random forest train ===")

    # Create a data set.
    train_df = pd.read_csv(config["train"]["train"])
    test_df  = pd.read_csv(config["train"]["test"])
    # Set the number of data types to be used for training.
    data_number = config["train"]["data_number"]
    # Extracting data
    X_train = train_df[train_df.columns[train_df.columns != train_df.columns[data_number]]].values
    Y_train = train_df[train_df.columns[data_number]]
    ...
    _ = output_result(forest, X_train, X_test, Y_test, config, save_folder_name)
